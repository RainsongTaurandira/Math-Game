...
function generateQA() {
    ...
    var x = y * (1 + Math.round(9*Math.random()));
    ...
    for (i=1; i<=4; i++){
        if (i !== correctPosition) {
            var wrongAnswer;
            do {
                var y1 = 1 + Math.round(9*Math.random());
                var x1 = y1 * (1 + Math.round(9*Math.random()));
                wrongAnswer = x1 / y1;
            } while (answers.indexOf(wrongAnswer) > -1)
            
            document.getElementById("box"+ i).innerHTML = wrongAnswer.toFixed(0);
